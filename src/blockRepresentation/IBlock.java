 ...
 * @author Francesco
 *
 * The memory representation of a program block
 */
public abstract class IBlock {
	 ...
	 */
	public void add_Previous( IBlock b) {
		m_previouses.add(b);
	}
	 ...
	 */
	public Set<IBlock> getAllNextsEver( Set<IBlock> s_already_in ) {
		...
		{
			if (  m_nexts.get(i).isWhileBlock() && s_already_in.contains(m_nexts.get(i)) )
				break;
			
			s.add(m_nexts.get(i));						
			s_already_in.add(m_nexts.get(i));	
					
			s.addAll(m_nexts.get(i).getAllNextsEver(s_already_in));
		}	
	 ...
	 * @return The set of successors Blocks
	 */
	public Set<IBlock> getAllPreviousesEver(  Set<IBlock> s_already_in ) {
		...
		{
			if (   s_already_in.contains(m_previouses.get(i)) )
				break;
			
			s.add(m_previouses.get(i));						
			s_already_in.add(m_previouses.get(i));	
					
			s.addAll(m_previouses.get(i).getAllPreviousesEver(s_already_in));
		}	
	...
	/** Checks whether the block implements IControlBlock , here the default answer "no", is defined. The sub classes answering "yes" will just override the method
	 * 
	...
	/** Checks whether the block antains any array reference, here the default answer "no", is defined. The sub classes answering "yes" will just override the method
	 * 
	 * @return true if it is, false otherwise
	 */
